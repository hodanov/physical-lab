...
- Model with build123d. Use FreeCAD for drawings, assembly checks, and
  analysis on the generated STEP files.
- Use millimeters for all dimensions.
- `cad/source/tsugu65/` contains the code-CAD source of truth; tests live in
  `cad/source/tests/`.
- `cad/export/` contains exports generated by `uv run tsugu65-build`; do not
  edit or commit them.
- `drawings/` contains FreeCAD TechDraw sources that reference the exported
  STEP files.
...
When changing a physical design, update the code-CAD source and its tests
first, regenerate the exports with `uv run tsugu65-build`, and record design or
manufacturing decisions in the product documentation. Update the FreeCAD
drawings at review or ordering milestones. When ordering, copy the exports used
into `manufacturing/<service>/<rev>/` and commit them.
